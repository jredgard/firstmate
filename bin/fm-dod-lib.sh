...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), run \`fm-pr-report-compact <org-url> <pr-id>\` once: Azure DevOps hard-truncates PR descriptions at 4000 characters and refuses edits after merge, so this is the only window to keep the full verification report readable. The tool posts a closed review-summary comment, with any residual findings in full, on every gated PR (the one predictable place reviewers read them), verifies the Build policy, normalizes the visible step board to final passed values, trims prose when needed, and never alters the attestation. Then append \`done: PR {url} checks green\` and stop. You are finished.
EOF
      ;;
    *)
