...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), run \`fm-pr-report-compact <org-url> <pr-id>\` once: Azure DevOps hard-truncates PR descriptions at 4000 characters and refuses edits after merge, so this is the only window to keep the full verification report readable. The tool posts the review's recorded findings as a closed PR comment on every gated PR (the one predictable place reviewers read them), compacts the description only when it overruns, and never alters the attestation. Then append \`done: PR {url} checks green\` and stop. You are finished.
EOF
      ;;
    *)
