...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  if [ "$WEDGE_ALARM_LAST_EPOCH" -gt 0 ] && [ $((now - WEDGE_ALARM_LAST_EPOCH)) -lt "$max_defer" ]; then
    ...
    log "ERROR: away-mode escalation undelivered ${age}s; last delivery failure: ${INJECT_LAST_FAILURE:-not recorded}; inject blocked (verdict=$blocker, detail=$detail). Buffer + wake-queue preserved; alarm marker written."
  fi
  {
...
inject_msg() {  # <message> [state]
  local msg=$1 state target backend retries sleep_s verdict composer encoded blocker bytes errf err='' 
  state="${2:-$(_state_root)}"
  inject_evidence_reset
  ...
  if ! fm_backend_target_exists "$backend" "$target"; then
    INJECT_LAST_FAILURE="supervisor target $target not found on $backend"
  ...
  case "$verdict" in
    ...
    send-failed) blocker=submit-send-failed ;;
  ...
  inject_evidence_record "$blocker" "submit-verdict=${verdict:-<empty>} after $retries retries${err:+: $err}" "$backend" "$target"
